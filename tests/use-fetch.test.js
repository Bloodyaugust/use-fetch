  ...
  describe('#execute', () => {
    ...
    it('provides the response as json', async () => {
      ...
      await act(async () => {
        await result.current.execute('https://test.com/posts')
          .then(({ json }) => {
            expect(json).toEqual({'posts': [{'content': 'lorem ipsum', 'created': '1/15/2021', 'user': 1}]});
          });
      });
    });
    ...
    it('rejects failed requests with response object, error, and mounted state', async () => {
      ...
      await act(async () => {
        await result.current.execute('https://test.com/404')
    ...
    it('cancels requests still in flight when unmounted and provides error object', async () => {
      ...
      await act(async () => {
        ...
        await result.current.execute('https://test.com/wait')
          .catch(({ error }) => {
